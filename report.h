...
******************************************************************************/
    ...
    struct S_Queue
    {
        uint8_t  m_ucaQueue[QUEUE_SIZE];  //!< The queue
        uint16_t m_uiQueueHead;           //!< The starting location in the queue
        uint16_t m_uiQueueTail;           //!< The ending location in the queue
        uint16_t m_uiQueueCount;          //!< The number of messages in the queue
    };
    ...
    uint16_t uiReport_RAM_QueueCount(void);

    void vReport_LogDE(uint8_t* ucBuf, uint8_t ucPriority);

    uint8_t ucReport_ReadDEFromRAM(uint8_t* p_ucaBuff);
